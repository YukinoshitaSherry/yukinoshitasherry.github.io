...
写的时候，但凡可以用关联词比如However, Nevertheless, In that case, Consequently, Therefore之类的，就抓紧用。

回答的时候，一个点一般是写一句阅读材料，写两句听力材料。

阅读和听力之间的内容怎么说转折呢，下面给了一堆表示否定的语料。

听力材料怎么写出两句呢，中间就用一个比如consequently之类的关联词连接起来，这样就能非常符合考核标准里的organization和coherence要求。

那么要怎么才有两句话好写呢，这就要在听听力的时候起码为每一个点记录下两句内容，他们直接有前后的逻辑关系。


不需要写总结句，但是词数要到250+比较稳妥，实际上现在写到300不成问题。
...
The reading passage posits/presents that ..., based on ...

However, the lecturer disputes this notion, offering some alternative explanations to argue that these evidences are insufficient and unconvincing.

Firstly, the reading passage contends that ..., which suggest that

Nevertheless, the lecturer refutes this opinion, stating that ... Therefore, ...

Secondly, the lecturer disagrees with the assertion in the reading passage that ...

The lecturer challenge the claim by contending that ... Concequently, ...

Lastly, while the reading passage presents evidence that ..., the lecturers presents a different perspective.

He/She explains that ... As such incidents were ..., it is not convincing that ... In that case, ...
...
The reading material contends/introduces/notes/points out/states that ...
...
#### xxf资料
先清晰表达自己的观点（回答教授的问题），然后下面给两个论据来证明自己的观点。


对于非常开放的问题，全部自己讲，不要和学生观点重复；对于二选一的问题，用just as Kelly mentioned去和学生互动。

看阅读材料的时候，教授的话倒着看，直接看他提的是什么问题；学生的话正着看，获取的是观点+理由。

凑字数，可以写一句客套话，不知道是不是有用
...
比如说这样: In today's dynamic business environment, effective leadership is essential for guiding companies to success  
...
It is indeed beneficial for sb. to do ...

First and foremost/To begin with, just as Kelly has said, ..., for ...

To be more specific/In other words/To elaborate, ...

For instance举个例子(翔实具体的故事讲述)/For example比如说(广为接受的事实陈述), ...

Therefore/Consequently/In that case, ...

说完理之后的举例子可以很简略，比如只罗列一些doing式的词组，或者一些很specific的内容
...
Undoubtedly, ... like Andrew said. However, ...
