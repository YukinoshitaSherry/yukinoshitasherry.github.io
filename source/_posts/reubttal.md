...
- openreview上会有字数限制，看各个会议的要求。
    - 如果限制5000字符等，事先统计好回答的字符数，然后按Response([1/n],[2/n]...,[n/n])切分，同一个weakness下尽量放一起。
    - 叠楼可以分别点offical comment的，最好别在一个offical comment叠楼。注意顺序，叠楼是倒序增加，但是最终呈现的字最好顺序。
    - 部分不允许叠楼的，全部问题要都回复；但先放主要的放全，其他次要的暂时略写；可以引导reviewer回复，说due to字数限制，如果您xxx，回复后继续补充/讨论。
