...
def bfs_graph(graph, start):
    ...
    while q:
        ...
        for v in graph[u]:
            if v not in seen:
                seen.add(v)            # 入队前标记，避免重复入队
                q.append(v)
    return order

def num_islands(grid):
    # Hot100-200：四连通岛屿数；原地把陆地改成水以当 visited
    if not grid:
        return 0
    ...
    for i in range(R):
        for j in range(C):
            if grid[i][j] == '1':
                dfs(i, j)              # 新岛屿：一次 DFS 抹平整块
                cnt += 1
    return cnt

def can_finish(numCourses, prerequisites):
    # Hot100-207：Kahn 拓扑判能否修完；prerequisites 中 [a,b] 表示 b→a（先修 b）
    g = defaultdict(list)
    indeg = [0] * numCourses
    ...
    while q:
        ...
        for v in g[u]:
            ...
            if indeg[v] == 0:
                q.append(v)            # 入度清零方可修
    return taken == numCourses         # 取出数不足说明有环
```
...
def has_cycle_undirected(n, g):
    ...
    for i in range(n):               # 处理非连通：每个分量各搜一次
...
def topo_dfs(n, g):
    ...
    WHITE, GRAY, BLACK = 0, 1, 2     # 未访 / 访问中 / 已完结
    color = [WHITE] * n
    order = []
...
def bfs_dist(g, start, n):
    # 单源无权最短路：dist[v] 为边数；-1 表示不可达
    dist = [-1] * n
    parent = [-1] * n                # 最短路上的前驱，便于还原路径
    q = deque([start])
    dist[start] = 0
    while q:
        u = q.popleft()
...
def restore_path(parent, t, start):
    ...
    while t != -1:
        ...
        if t == start:
            break
        t = parent[t]
    path.reverse()                   # 回溯得到的是逆序，反转成 start→…→t
    return path

def bfs_grid(grid, sr, sc, tr, tc):
    """网格最短路：0 可走、1 障碍；四向移动；返回步数，不可达 -1。"""
    R, C = len(grid), len(grid[0])
    if grid[sr][sc] == 1:
        return -1                    # 起点是障碍
...
def word_ladder_len(beginWord, endWord, wordList):
    """Hot100-127：单词接龙；每次改一个字母，BFS 求最短变换次数。"""
    bank = set(wordList)             # O(1) 查询合法词
    if endWord not in bank:
        return 0
    q = deque([(beginWord, 1)])      # (当前词, 已用步数/词数)
    seen = {beginWord}
    while q:
        w, d = q.popleft()
        if w == endWord:
...
/* 无权图 BFS：返回 (dist, parent)；dist[v]==-1 不可达 */
...
int numIslands_bfs(vector<vector<char>>& grid) {
    ...
    int dr[4] = {0, 0, 1, -1}, dc[4] = {1, -1, 0, 0};  // 四向
    for (int i = 0; i < R; i++)
        for (int j = 0; j < C; j++) {
