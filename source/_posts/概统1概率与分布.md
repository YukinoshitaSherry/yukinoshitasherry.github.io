...
> $$
> 当 $n\to\infty$ 时，$\prod_{j=0}^{k-1}\left(1-\frac{j}{n}\right)\to 1$，故 $\binom{n}{k}\sim \frac{n^k}{k!}$。
> 2) 因为 $np_n\to\lambda$，所以 $n^k p_n^k\to \lambda^k$。
> 3) 处理 $(1-p_n)^{n-k}$：
> $$
...
> [!INFO]+ 推导：归一化常数 $\frac{1}{\sqrt{2\pi}\sigma}$ 从哪里来
> 先处理标准正态的积分
> $$
