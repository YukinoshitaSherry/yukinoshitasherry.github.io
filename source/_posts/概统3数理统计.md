...
>
> 三步走（做题流程）：
> 1) **在估什么**（$\mu$、$\sigma^2$、$p$、$\lambda$…）以及样本来自什么模型；  
> 2) **选方法**：MOM（用矩匹配）、MLE（最大化似然）、或用抽样分布/枢轴量做区间；  
> 3) **评价好不好**：无偏/相合/MSE/有效性/渐近正态性（看题目问什么）。
>
...
>
> 四步走（做题流程）：
> 1) **写清楚 $H_0/H_1$**（单侧/双侧；简单/复合）。  
> 2) **选统计量 $T$**（来自抽样分布：$Z/t/\chi^2/F$，或 LRT/NP）。  
> 3) **先定 $\alpha$ 再定拒绝域**（或等价地算 p 值）。  
> 4) **代入样本值下结论**（并解释一类/二类错误与功效的含义）。
>
...
<div style="display: flex; gap: 0; margin: 10px 0;">
  <img src="https://raw.githubusercontent.com/YukinoshitaSherry/qycf_picbed/main/img/20260318012925761.png" style="width: 45%; height: auto; display: block;">
  <<img src="https://raw.githubusercontent.com/YukinoshitaSherry/qycf_picbed/main/img/20260318012932153.png" style="width: 45%; height: auto; display: block;">
</div>
<div style="display: flex; gap: 0; margin: 10px 0;">
  <img src="https://raw.githubusercontent.com/YukinoshitaSherry/qycf_picbed/main/img/20260318012938889.png" style="width: 45%; height: auto; display: block;">
  <img src="https://raw.githubusercontent.com/YukinoshitaSherry/qycf_picbed/main/img/20260318012946460.png" style="width: 45%; height: auto; display: block;">
</div>

> 图源右下角水印，小红书<a href="https://www.xiaohongshu.com/user/profile/695676df000000002a036760?xsec_token=ABcLBQ9aAcuNbs9kJIkuydc9Z-aoDnVQVkQRmalUQLZJA">@数据科学家阿宝哥</a>
...
> [!INFO]+ 直观理解（回归 & ANOVA）
> - **回归**：用“直线/超平面”描述 $x$ 与 $y$ 的平均关系，核心是**最小二乘 = 把 $\mathbf{Y}$ 投影到 $\mathrm{span}(\mathbf{X})$ 上**。  
> - **方差分析**：比较“组间差异”与“组内波动”，核心是**把总波动分解**：
>   $$
...
<img src="https://raw.githubusercontent.com/YukinoshitaSherry/qycf_picbed/main/img/20260318012353106.png" style="width:70%"><br>
...
<div style="display: flex; gap: 0; margin: 10px 0;">
  <img src="https://raw.githubusercontent.com/YukinoshitaSherry/qycf_picbed/main/img/20260318013210207.png" style="width: 26%; height: auto; display: block;">
  <img src="https://raw.githubusercontent.com/YukinoshitaSherry/qycf_picbed/main/img/20260318013230714.png" style="width: 72%; height: auto; display: block;">
</div>
...
> [!INFO]+ 推导：$\mathrm{SST}=\mathrm{SSB}+\mathrm{SSW}$
> 对每个观测值做“加减组均值”的恒等分解：
> $$
...
> [!INFO]+ 操作步骤（按图走一遍）
> 1) 写假设：$H_0:\mu_1=\cdots=\mu_k$；$H_1$：至少一组不同。  
> 2) 算三类平方和：$\mathrm{SST},\mathrm{SSB},\mathrm{SSW}$（并检查分解恒等式）。  
> 3) 算自由度：$\mathrm{df}_B,\mathrm{df}_W$，进而 $\mathrm{MSB}=\mathrm{SSB}/\mathrm{df}_B$，$\mathrm{MSW}=\mathrm{SSW}/\mathrm{df}_W$。  
> 4) 统计量：$F=\mathrm{MSB}/\mathrm{MSW}$；查 $F_{1-\alpha}(k-1,N-k)$ 或看 p 值。  
> 5) 结论：若拒绝 $H_0$，只能说明“至少有一组均值不同”，要回答“哪几组不同”需做事后检验。
