...
本文不是“泄露事件记录”，而是基于可运行逆向仓库、飞书长文笔记、社区逆向观察与外部论文证据，对 Claude Code 运行时进行结构化重建。分析目标是抽取可迁移的工程模式，而非复述单一材料。

核心方法：

- 生命周期对齐：把模块放回 runtime loop 观察。
- 模块边界对齐：区分 memory、调度、安全、扩展、交互等职责。
- 证据分层：代码路径线索优先，社区观察作为补充。

在“源码提取”环节采用统一口径，避免社区统计差异造成误解：

- 先核对 npm 包内 `cli.js` 与 `cli.js.map` 是否存在。
- 再分别统计 `sources` 原始条目数与去重后可写回文件数。
- 将“可解析条目总量”与“可复原源码文件量”分开报告，不混用。

这一步的目的不是追求某个单一文件数结论，而是确认“可恢复的未混淆源码规模足以支持工程级链路分析”。
...
高质量工程 Prompt 通常分为四层，每层负责不同失败模式：
...
工程上常见问题是把四层混写，导致“规则很多但执行不稳”。推荐写法是每层独立成块，避免规则互相覆盖。
