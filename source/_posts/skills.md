...
>
> - **Tool**：锤子、螺丝刀（最低层工具）
> - **Skill**：会「安装书架」这门手艺（组合使用多种工具完成目标）-> 工具箱+说明书+经验等等
...
- 标题尽量短小精悍：优先“对象 + 关键词”，避免长句。
- **不缩写术语/概念**：例如“单调性”不要写成“单调”，“一致连续性”不要写成“一致连续”。
- 章节名用“定义 / 定理 / 推论 / 性质 / 例题”这类稳定标签，方便检索。

### 11.2 正文

- 正文只放**定义**、**定理/命题（只陈述）**、**结论列表**与必要的符号约定。
- 不在正文夹带聊天口吻（例如“后面会用到”“先给路线图”这类叙述），保持笔记可独立阅读。

### 11.3 推导进卡片

- 推导/证明/计算过程统一放在 callout 里：
  - `> [!INFO]+`：逐步推导、证明、关键不等式链
  - `> [!EXAMPLE]`：例题与逐步解法
- 原则：**先看到结论，再在卡片里展开推导**。
- 数学公式统一用 `$...$`（行内）与 `$$...$$`（块公式）；不要用 `\[` `\]`。
- **矩阵换行**：在本博客渲染环境下，矩阵环境（如 `pmatrix/bmatrix`）行分隔建议写成 `\\\\`（四个反斜杠）以确保换行生效（例如 `a & b \\\\`）。
- **英语专有名词**：只在确有专有名词时（例如 Dedekind cut / Supremum axiom）在标题下单独加一行英文；一般术语不必附英文。

### 11.4 增量更新不影响其它

- 每次只增量完善一个小节，保留其他小节结构不动。
- 分隔符用 `<br>`（项目约定），不要用 `---`（除 frontmatter）。
