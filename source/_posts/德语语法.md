...
- **单数**（Singular）：使用相应的定冠词（der/die/das），如：der Mann（男人）、die Lampe（灯）、das Kind（孩子）、das Auto（汽车）
- **复数**（Plural）：复数形式统一使用定冠词 **die**，但词尾会有变化，如：die Männer（男人们）、die Lampen（灯们）、die Kinder（孩子们）、die Autos（汽车们）
