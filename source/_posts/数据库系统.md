...
| 2NF | Second Normal Form | 非主属性对码的**部分依赖** | 非主列是否只依赖复合键的一半 | 通常先拆部分依赖 |
| 3NF | Third Normal Form | 非主属性对码的**传递依赖** | 非主列是否经另一非主列才连到键 | 可**无损 + 保持依赖** |
...
> 2. 有没有非主属性只依赖码的真子集？→ 有则非 2NF。
> 3. 有没有非主属性传递依赖码？→ 有则非 3NF。
...
**搜索码（Search Key）** ：用于查找的属性（集）。**索引项（Index Entry）** ：`(search-key, pointer)`。索引文件通常远小于数据文件。
...
| 类型 | 含义 |
| :--- | :--- |
| 主索引 / 聚集索引（Primary / Clustering Index） | 搜索码顺序与文件物理顺序一致 |
| 辅助索引 / 非聚集索引（Secondary / Nonclustering Index） | 搜索码顺序与文件顺序不同 |
...
用哈希函数把搜索码映射到桶。等值点查平均 $O(1)$，**范围查询弱**。静态哈希桶数固定；**动态哈希（Dynamic Hashing）**  / **可扩展哈希（Extendable Hashing）** 随数据量调整。教材强调：哈希索引通常作**辅助索引**形态出现（具体产品另论）。
